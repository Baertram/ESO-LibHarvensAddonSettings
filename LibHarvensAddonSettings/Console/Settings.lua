...
--v- Baertram
local LHAS_settingsEntryInGameMenu = LibHarvensAddonSettings.LHAS_settingsEntryInGameMenu
--ReloadUI dialog
local showSettingsMenu = LibHarvensAddonSettings.ShowSettingsMenu
local showReloadUIConfirmationDialog = LibHarvensAddonSettings.ShowReloadUIConfirmationDialog
local getLHASActivePanel = LibHarvensAddonSettings.GetLHASActivePanel
local buildRequiresReloadLabelText = LibHarvensAddonSettings.BuildRequiresReloadLabelText
--^- Baertram
...
local updateControlFunctions = {
	[LibHarvensAddonSettings.ST_CHECKBOX] = function(self, control)
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(buildRequiresReloadLabelText(self, self.labelText)))) --Baertram
	...
	[LibHarvensAddonSettings.ST_BUTTON] = function(self, control)
		...
		control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(buildRequiresReloadLabelText(self, self.labelText)) or self:GetValueOrCallback(buildRequiresReloadLabelText(self, self.labelText)))) --Baertram
	...
	[LibHarvensAddonSettings.ST_COLOR] = function(self, control)
		...
		label:SetText(self:GetString(self:GetValueOrCallback(buildRequiresReloadLabelText(self, self.labelText)))) --Baertram
...
local setupControlFunctions = {
	[LibHarvensAddonSettings.ST_CHECKBOX] = function(self, params)
		...
		self.requiresReload = params.requiresReload --Baertram
	end,
...
--v- Baertram
				if LibHarvensAddonSettings.reloadUIRequested then
					return showReloadUIConfirmationDialog(getLHASActivePanel())
				end
--^- Baertram
		...
		table.insert(
			...
			CreateEntry(
				"LibHarvensAddonSettings",
				{
					...
					name = LHAS_settingsEntryInGameMenu, 							--Baertram
	...
	scene:SetHideSceneConfirmationCallback(function(scene, nextSceneName, bypassHideSceneConfirmationReason)
        if not bypassHideSceneConfirmationReason then
            showReloadUIConfirmationDialog(getLHASActivePanel())
        else
