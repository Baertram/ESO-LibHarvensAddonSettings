...
local updateControlFunctions = {
	[LibHarvensAddonSettings.ST_CHECKBOX] = function(self, lastControl)
		...
		self.control:GetNamedChild("Name"):SetText(self:GetString(self:GetValueOrCallback(buildRequiresReloadLabelText(self, self.labelText)))) --Baertram
...
function LibHarvensAddonSettings:CreateAddonSettingsPanel()
	ZO_OptionsWindow_AddUserPanel("HarvensAddonSettingsPanel", LHAS_settingsEntryInGameMenu, PANEL_TYPE_SETTINGS) 	--Baertram
...
--v- Baertram
local function GameMenuSceneStateChange(oldState, newState)
	if newState == SCENE_SHOWING then
		LibHarvensAddonSettings:Initialize()
	elseif newState == SCENE_HIDING then
		showReloadUIConfirmationDialog(GAME_MENU_SCENE, getLHASActivePanel())
	end
end
GAME_MENU_SCENE:RegisterCallback("StateChange", GameMenuSceneStateChange)
--^- Baertram
